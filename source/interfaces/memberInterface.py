...
def register():
    ...
    if first_name == "" or last_name == "" or height == "" or weight == "" or goals == "" or routines == "": 
        ...
        return
    member_id = util.register_member(first_name, last_name, height, weight, goals, routines)
    print(
...
def updateProfile(member_id):
    ...
    while True:
        try:
            ...
            goals = input("Fitness Goals: ")
            routines = input("Exercise Routines: ")
            util.update_member_profile(member_id, first_name, last_name, height, weight, goals, routines)
            break
        except:
...
def displayDashboard(member_id):
    ...
    while True:
        ...
        if selection == "1":
            print(f"\nFitness Goals: {util.fetchGoals(member_id)}")
        elif selection == "2":
            print(f"\nExercise Routines: {util.fetchRoutines(member_id)}")
        elif selection == "3":
            managePTSessions(member_id)
        elif selection == "4":
            fitnessClasses = util.fetchMemberClasses(member_id)
    ...
    while True:
        ...
        if selection == "1":
            ...
            try:
                ...
                util.schedule_session(member_id, trainerID, date, sTime, eTime)
            except:
...
def registerClasses(member_id):
    ...
    while True:
        ...
        if selection == "1":
            classes = util.fetchFitnessClasses()
            for fitclass in classes:
                print(
                    f"\nFitness Class ID {fitclass[0]}: {fitclass[1]} scheduled for {fitclass[2]} from {fitclass[3]} to {fitclass[4]}"
                )
        elif selection == "2":
            classes = util.fetchClassIDs()
            class_id = input("\nEnter the class ID you would like to register for: ")
            if not class_id in classes:
                print("\nClass ID does not exist.")
            else:
                if not util.isRegistered(member_id, class_id):
                    util.registerForClass(member_id, class_id)
...
def managePTSessions(member_id):
    ...
    while True:
        ...
        if selection == "1":
            ptSessions = util.fetchPTSessions(member_id)
...
def reschedulePTSession(member_id):
    ...
    try:
        session_exists = util.checkPTSessionExists(member_id, session_id)
    except:
    ...
    if session_exists[0][0] == 0:
        ...
        while True:
            trainerID = util.findTrainerForSession(session_id)
            ...
            if util.checkDateTimeValidity(
                date, startTime, endTime
            ) and util.trainer_is_available(trainerID, date, startTime, endTime):
                break
            else:
                print("\nPlease enter a valid date and time. ")
        util.reschedulePTSession(date, startTime, endTime, session_id)
        print(
...
def cancelSession(member_id):
    ...
    if session_exists[0][0] == 0:
        ...
        util.deletePTSession(member_id, session_id)
